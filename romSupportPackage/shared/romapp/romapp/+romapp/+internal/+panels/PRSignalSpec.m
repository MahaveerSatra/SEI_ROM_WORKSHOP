classdef PRSignalSpec <  matlab.mixin.SetGet
    %
    ...
    properties(SetAccess=private)
        ...
        SignalTable
        Widgets
        Spec romapp.internal.data.PRSignalSpec
    end
    ...
    methods
        function this = PRSignalSpec(tool,spec,parent,row,col)
            % PRSIGNALSPEC
        ...
        function updateSpec(this,varargin)
            ...
            npw = this.Widgets.edtNumPulse.Value;
            ranges = cell2mat(this.SignalTable.Data(:,2:3));
            if nargin == 1
                setValues(this.Spec,pw,npw,ranges); % update signal spec of this panel
            elseif nargin>1
                setValues(varargin{1},pw,npw,ranges); % update a standalone signal spec
            end
        end
    ...
    methods (Access=private)
        function buildPanel(this,parent,row,col)
            ...
            editFieldNPW = uispinner(layoutOptions);
            editFieldNPW.Layout.Row = 2;
            editFieldNPW.Layout.Column = 2;
            editFieldNPW.Limits = [1 inf];
            editFieldNPW.RoundFractionalValues = 'on';
            editFieldNPW.UpperLimitInclusive = 'off';
            editFieldNPW.Value = 10;
            labelSignalLimits = uilabel(layoutOptions);
            labelSignalLimits.Text = romapp.internal.resources.getString('lblPRSignalSpec_Limits');
            labelSignalLimits.Layout.Row = 3;
            labelSignalLimits.Layout.Column = 1;
            signalTable = uitable(layoutOptions);
            signalTable.Layout.Row = 4;
