classdef PRSignalSpec < romapp.internal.data.SignalSpec
    %
    ...
    properties(Constant)
        TYPE string = "PRPS";
        DESCRIPTION string = romapp.internal.resources.getString('lblPRPS_Description')
    end
    ...
    methods
        function obj = PRSignalSpec(signals)
        ...
        function obj = copy(this)
            ...
            obj = romapp.internal.data.PRSignalSpec(this.Signals);
    ...
    methods
        ...
        function values = sampleSpecValues(this,nump)
            ...
            ranges = this.Ranges;
            for ct = 1:nSig
                values(:,ct) = ranges(ct,1) + diff(ranges(ct,:))*rand(nump,1);
            end
        end
    end
